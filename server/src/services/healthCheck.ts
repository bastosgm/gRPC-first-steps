...
const healthCheck = async (
  ...
  if (call.request.HealthCheck === "health") {
    ...
    const nativeApi = await fetchPosts();
    ...
    logger.log(
      "debug",
      `Request made successfully: health | ${JSON.stringify(
        res,
      )} | ['Health', 'case', 'theHiveData', 'data']`,
    );
  } else {
    callback(
      {
    ...
    logger.log(
      "error",
      `Invalid message! probably it's a typo which does not match with \'{HealthCheck: health}\'.`,
